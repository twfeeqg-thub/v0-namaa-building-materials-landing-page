...
export default function ChatWindow({ isOpen, onClose }: { isOpen: boolean; onClose: () => void; }) {
  const [messages, setMessages] = useState<Message[]>([
    { sender: 'assistant', text: "مرحباً بك. أنا مساعدك الذكي. كيف يمكنني مساعدتك اليوم؟" }
  ]);
  const [inputValue, setInputValue] = useState('');

  useEffect(() => {
    if (isOpen) {
      playWhooshSound(); // تشغيل صوت الانبثاق
    }
  }, [isOpen]);

  const handleSendMessage = () => {
    if (inputValue.trim() === '') return;
    playSendSound(); // تشغيل صوت الإرسال
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundColor: 'rgba(34, 75, 88, 0.8)', // تغيير اللون ليتناسب مع الهوية البصرية
